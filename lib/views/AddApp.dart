...
class AddApp extends StatefulWidget {
  ...
  const AddApp({super.key, required this.appsSeleccionadas});
...
class _AddAppState extends State<AddApp> {
  ...
  void initState() {
    ...
    for (var app in todasLasApps) {
      for (var seleccionada in widget.appsSeleccionadas) {
        if (app.appKey == seleccionada.appKey) {
          app.seleccionada = true;
          app.minutos = seleccionada.minutos;
          app.tiempoInicio = seleccionada.tiempoInicio;
        }
      }
    }
  }
  ...
  void guardar() {
    final seleccionadas = todasLasApps
        .where((app) => app.seleccionada)
        .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Añadir Apps", style: TextStyle(color: Colors.black)),
